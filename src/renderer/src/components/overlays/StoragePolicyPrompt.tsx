...
// The storage question — whether video may be buffered to disk.
//
...
//
// On a fresh install the question is asked INSIDE the welcome flow
// (WelcomeSetup's storage step renders StoragePolicyChoice below), so this
// standalone prompt is normally dormant. It stays mounted as the safety
// net for any state where the question is somehow still unanswered after
// setup — the cost of an unanswered policy (video written to a disk that
// may be protected) is the one this app refuses to risk.
//
// SHOWN ONLY WHEN GENUINELY UNANSWERED. That is why the snapshot carries
// storagePolicyChosen separately from storeMedia: once the flag is a
...
/**
 * The question's body, options and answer buttons — everything except the
 * chrome around it. Shared between this standalone prompt and WelcomeSetup's
 * storage step so the copy can never drift between the two places it is
 * asked. The host owns the IPC call; this only reports which was chosen.
 */
export function StoragePolicyChoice({
  busy,
  onChoose
}: {
  busy: boolean
  onChoose: (storeMedia: boolean) => void
}) {
  return (
    <>
      <p className={styles.body}>
        While something plays, R3 buffers it ahead so you can rewind and resume without re-fetching.
        That buffer can live on your disk, or only in memory.
      </p>
      <ul className={styles.points}>
        ...
        <li>
          <strong>Stream only</strong> — nothing about what you watch is written to disk. A shorter
          buffer, and a faster connection helps.
        </li>
      </ul>
  ...
  return (
    <div className={styles.scrim} role="presentation">
      ...
      >
        ...
        <StoragePolicyChoice busy={busy} onChoose={(storeMedia) => void answer(storeMedia)} />
      </div>
    </div>
  )
}
