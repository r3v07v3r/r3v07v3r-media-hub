...
const {filterCatalog,isItemWatched,dedupeCatalog,episodeWatchState,airingStatus,subtitlesInadequate,continueWatchingList,createRoomCode,rankStreams,validateTorBoxToken,meteorConfigPath,meteorP2PConfigPath,normalizeMeta,normalizeKitsuAnime,normalizeKitsuEpisode,normalizeSimklCatalog,normalizeSimklSearchResult,filterAnimeRelationships,normalizeTmdbCollectionPart,parseReleaseName,enrichTorBoxItem,selectPlayableStream,selectVideoFile}=require('../src/core.cjs');
...
test('Kitsu ratings are converted from its 0-100 scale to the same 0-10 scale used everywhere else, so the rating filter and display are consistent across movies, series and anime',()=>{assert.equal(normalizeKitsuAnime({id:'1',attributes:{averageRating:'84.48'}}).rating,'8.4');assert.equal(normalizeKitsuAnime({id:'2',attributes:{averageRating:78.46}}).rating,'7.8');assert.equal(normalizeKitsuAnime({id:'3',attributes:{}}).rating,'')});
test('a live-search text query bypasses genre rating and year filters, since search results are explicitly requested by name and often lack the metadata those facets need',()=>{const items=[{title:'Prince of Tennis',year:'2001',type:'anime',rating:'7.8',genres:[]},{title:'Recent Show',year:'2024',type:'anime',rating:'9.5',genres:['Action']}];const filtered=filterCatalog(items,'anime','prince',{includeGenres:['Action'],minRating:9,minYear:2020});assert.deepEqual(filtered.map(x=>x.title),['Prince of Tennis']);assert.deepEqual(filterCatalog(items,'anime','',{includeGenres:['Action'],minRating:9,minYear:2020}).map(x=>x.title),['Recent Show'])});
...
test('subtitles are treated as inadequate when there are none, or every track is signs/songs-only (zxx language or a signs/songs label), but not when a real dialogue track exists',()=>{
 assert.equal(subtitlesInadequate([]),true);
 assert.equal(subtitlesInadequate(undefined),true);
 assert.equal(subtitlesInadequate([{language:'zxx',label:'Signs & Songs • ZXX • ASS'}]),true);
 assert.equal(subtitlesInadequate([{language:'',label:'Songs • SUBRIP'}]),true);
 assert.equal(subtitlesInadequate([{language:'eng',label:'English • ENG • SUBRIP'}]),false);
 assert.equal(subtitlesInadequate([{language:'zxx',label:'Signs'},{language:'eng',label:'English • ENG • SUBRIP'}]),false);
});
