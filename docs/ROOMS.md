...
- **memberKey** — a random identity each install generates per room. It
  is sent on connect and carried in presence announcements, which is how
  clients know which keys belong to which person.
...
The admin kicks a person, not a device: every memberKey their announcements
ever carried is kicked together. In order — and the order is the
guarantee:
...
(known memberKey), and announces under the old secret. Any re-keyed
member that hears them re-sends the re-key under the old secret — safe,
because the one party that must not hear it cannot connect. Nobody has
...
- **A device of theirs the room never saw** (never announced, never
  ...
  refused. But if it was *connected and silent* at kick time under a
...
- **Within-room identity is honest, not enforced.** Anyone holding the
  ...
  against them. The boundary against *outsiders* is the secret itself;
  the boundary against *kicked members* is the relay ban plus the
  rotation.
